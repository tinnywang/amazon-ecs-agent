...
amazon-linux-sources.tgz:
	...
	tar -czf ./sources.tgz ecs-init scripts misc agent amazon-ecs-cni-plugins amazon-vpc-cni-plugins agent-container ecs-version Makefile VERSION GO_VERSION RELEASE_COMMIT
...
.generic-rpm-integrated-done: get-cni-sources update-version
	...
	tar -czf ./sources.tgz ecs-init scripts misc agent amazon-ecs-cni-plugins amazon-vpc-cni-plugins agent-container ecs-version Makefile VERSION GO_VERSION
	test -e SOURCES || ln -s . SOURCES
